...
def calculate_mini_akm_link_velocity_steering(data):
    pub_vel_mini_akm_base_footprint_velocity=rospy.Publisher('mini_akm_base_footprint_velocity/link_cmd_vel', Float64,queue_size=1)
    pub_vel_mini_akm_base_footprint_steering=rospy.Publisher('mini_akm_base_footprint_steering/link_cmd_vel', Float64,queue_size=1)
    v=data.twist
    if(len(v) == 18):     
...
def calculate_mini_akm_joint_velocity_steering(data):
    pub_vel_mini_akm_left_rear_wheel = rospy.Publisher('mini_akm_left_rear_wheel_velocity/cmd_vel', Float64, queue_size=1)
    pub_vel_mini_akm_right_rear_wheel = rospy.Publisher('mini_akm_right_rear_wheel_velocity/cmd_vel', Float64, queue_size=1)
    pub_pos_mini_akm_left_steering_hinge = rospy.Publisher('mini_akm_left_steering_hinge/cmd_vel', Float64, queue_size=1)
    pub_pos_mini_akm_right_steering_hinge = rospy.Publisher('mini_akm_right_steering_hinge/cmd_vel', Float64, queue_size=1)
    a=data.velocity
    b = data.position
...
def Pub_mini_akm_real_cmd_vel():
    ...
    rospy.Subscriber("/"+robot_frame+"/joint_states", JointState, calculate_mini_akm_joint_velocity_steering)
